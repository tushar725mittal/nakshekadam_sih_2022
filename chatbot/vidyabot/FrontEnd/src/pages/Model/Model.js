...
function Model() {
  ...
  useEffect(() => {
    if (selectedOpt.value === "en-IN") {
      ...
      setCheckEng(false)
    }else if (selectedOpt.value === "ka-IN") {
      setCheckEng(false)
    }else if (selectedOpt.value === "ta-IN") {
      setCheckEng(false)
    }
  }, [selectedOpt])
  ...
  const api_response = async () => {
    ...
    else if (mic.lang === "ka-IN") { setMeta(data.documents[0].answer_ka); }
    else if (mic.lang === "ta-IN") { setMeta(data.documents[0].answer_ta); }
  ...
  useEffect(() => {
    ...
    if (popupData)
      if (selectedOpt.value === "en-IN") {
        ...
        setMeta(popupData[0].answer_ka)
      }else if (selectedOpt.value === "ta-IN") {
        setMeta(popupData[0].answer_ta)
      }
        ...
        <AlertDialogOverlay>
          ...
          <div className="finish-btn">
            {meta ? <Select options={actions} defaultValue={() => {
              ...
              else if (mic.lang === "ka-IN") { return actions[3] }
              else if (mic.lang === "ta-IN") { return actions[4] }
